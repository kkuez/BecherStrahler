    ...
    double zurück=0;
                        while(zurück==0){
                            try {//Line 1 ist der LDR
                                zurück = Double.parseDouble(messProcess.getMessLinesFürJedenEingang().get(1).substring(messProcess.getMessLinesFürJedenEingang().get(1).indexOf("(") + 1, messProcess.getMessLinesFürJedenEingang().get(1).indexOf(".") + 4));
                            }catch (Exception e){
                                System.out.println("Fehler beim Parsen von Pin 0"+pin);
                                e.printStackTrace();
                            }
                            }
                            messProcess.getMessLinesFürJedenEingang().set(pin, "0");
    return zurück;
}
public void kalibieren(int pinnummer){
    ...
    while(messProcess.getMessLinesFürJedenEingang().size()<8){
  ...
  for(int i =0;i<20;i++) {
    ...
    PinHub.setMessListe(pinnummer,messZustandsListe);
